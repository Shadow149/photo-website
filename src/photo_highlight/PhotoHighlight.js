...
function PhotoHighlight() {
  ...
  const getPhotoData = (ptitle) => {
    axios
    .get("https://alfredroberts.netlify.app/.netlify/functions/server/record/title/" + ptitle)
    .then((response) => {
      ...
      setTime(response.data[0].metaData.time !== undefined ? response.data[0].metaData.time : '');
      setLoading(false);
    })
  ...
  const sunDial = () => {
    if (time == '' || location.lat == '' || location.lng == '') {
      return null;
    }
    ...
    let sunTimes = SunCalc.getTimes(date, location.lat, location.lng);
    let sunRise = sunTimes.sunrise.getHours() + (sunTimes.sunrise.getMinutes() / 60)
    let sunSet = sunTimes.sunset.getHours() + (sunTimes.sunset.getMinutes() / 60)

    const mathTime = hour + (minutes / 60);
    const sunX = (((mathTime - sunRise)/(sunSet - sunRise)) * 80);
    const sunY = 40 - Math.sqrt((40**2) - ((sunX - 40) ** 2));
